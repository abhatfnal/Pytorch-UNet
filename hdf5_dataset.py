...
#         # Open the file and get the available groups
#         with h5py.File(self.files_img[fileno], 'r') as h5_file:
#             available_groups = list(h5_file.keys())  # e.g., ['0', '1', '2', '3', '4']
#             available_groups = [int(group) for group in available_groups]  # Convert to integers
#             available_groups.sort()  # Ensure groups are sorted
    
#             # Check if the requested index is valid
#             if this_ID not in available_groups:
#                 raise IndexError(f"Index {this_ID} out of range. Available groups: {available_groups}")
...
class HDF5Dataset(Dataset):
    def __init__(self, files_img, files_mask, img_tags, mask_tags, indices, rebin, x_range, y_range, z_scale, truth_th):
        ...
        self.img_tags = img_tags
        self.mask_tags = mask_tags
        self.indices = indices  # Expect a list of (file_index, group_index)
        self.rebin = rebin
        self.x_range = x_range
        self.y_range = y_range
        self.z_scale = z_scale
    ...
    def __getitem__(self, idx):
        file_idx, group_idx = self.indices[idx]
        file_img = self.files_img[file_idx]
        file_mask = self.files_mask[file_idx]

        imgs = list(h5u.get_chw_imgs(file_img, [group_idx], self.img_tags, self.rebin, self.x_range, self.y_range, self.z_scale))[0]
        masks = list(h5u.get_masks(file_mask, [group_idx], self.mask_tags, self.rebin, self.x_range, self.y_range, self.truth_th))[0]
