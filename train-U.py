...
def eval_img(net, dataset, gpu=False):
    for i, b in enumerate(dataset):
        ...
        # Check if img and true_mask are tensors, and convert them to NumPy arrays if necessary
        ...
        # Get the predicted mask from the network
        mask_pred = net(img)[0]

        # Detach the tensors from the graph before converting to NumPy
...
def train_net(net,
    ...
    print("Train dataset indices:", iddataset['train'], file=outfile_log, flush=True)
    print("Validation dataset indices:", iddataset['val'], file=outfile_log, flush=True)
    print("Test dataset indices:", iddataset['test'], file=outfile_log, flush=True)
    ...
    # Load test dataset (from the same sample/target sets)
    test_dataset = HDF5Dataset(
        files_img=files_img,
        files_mask=files_mask,
        img_tags=im_tags,
        mask_tags=ma_tags,
        indices=iddataset['test'],
        rebin=img_scale,
        x_range=x_range,
        y_range=y_range,
        z_scale=z_scale,
    ...
    print(f"Test dataset indices: {iddataset['test']}")
    print(f"Test image tags: {im_tags}")
    print(f"Test mask tags: {ma_tags}")
    ...
    for epoch in range(sepoch, sepoch + nepoch):
        ...
        net.eval()
        with torch.no_grad():
            print("Starting validation phase...")  # Add this print before the validation loop
            val_dice, val_loss = eval_dice_loss(net, val_loader, criterion, gpu)
            print("Validation phase completed.")  # Add this print after the eval_dice_loss call
        ...
        for batch_idx, (imgs, true_masks) in enumerate(test_loader):
            print(f"Processing test batch {batch_idx + 1}/{len(test_loader)}")
            if batch_idx == 380:  # Stop after a few batches to check
                break
        ...
        # for imgs, true_masks in test_loader:
            ...
            masks_pred = net(imgs)
            masks_pred_probs = masks_pred.cpu().detach().numpy()
            true_masks_np = true_masks.cpu().detach().numpy()

            ep_p0 = eval_eff_pur(net, zip(imgs, true_masks), 0.5, gpu)
            test_perf_p0['plane_0'] = ep_p0
            print(f'plane 0, ep: {ep_p0}', file=outfile_ep, flush=True)
            writer.add_scalar(f'test/plane0-pixel_eff', ep_p0[0], epoch)
            writer.add_scalar(f'test/plane0-pixel_pur', ep_p0[1], epoch)

            true_img, pred_img = eval_img(net, zip(imgs, true_masks), gpu)
            test_true_img, test_pred_img = log_fig(true_img, pred_img)
            writer.add_figure(f"test/plane0-true", test_true_img, epoch)
            writer.add_figure(f"test/plane0-pred", test_pred_img, epoch)

            ep_p1 = eval_eff_pur(net, zip(imgs, true_masks), 0.5, gpu)
            test_perf_p1['plane_1'] = ep_p1
            print(f'plane 1, ep: {ep_p1}', file=outfile_ep, flush=True)
            writer.add_scalar(f'test/plane1-pixel_eff', ep_p1[0], epoch)
            writer.add_scalar(f'test/plane1-pixel_pur', ep_p1[1], epoch)

            true_img, pred_img = eval_img(net, zip(imgs, true_masks), gpu)
            test_true_img, test_pred_img = log_fig(true_img, pred_img)
            writer.add_figure(f"test/plane1-true", test_true_img, epoch)
            writer.add_figure(f"test/plane1-pred", test_pred_img, epoch)

        test_perf_p0 = pd.DataFrame.from_dict(test_perf_p0)
        test_perf_p1 = pd.DataFrame.from_dict(test_perf_p1)
        eff_img, pur_img = ep_fig(test_perf_p0, test_perf_p1, ['plane_0', 'plane_1'])
        writer.add_figure("test/pixel_eff", eff_img, epoch)
        writer.add_figure("test/pixel_pur", pur_img, epoch)
