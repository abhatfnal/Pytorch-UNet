...
def eval_eff_pur(net, data, mask_threshold, gpu):
    eff_pix = 0
    pur_pix = 0
    eff_roi = 0
    pur_roi = 0
    ...
    if not data:
        print("Warning: Empty dataset. Returning NaN for efficiency and purity metrics.")
    ...
    for i, (img, mask) in enumerate(data):
        ...
        # Calculate metrics
        eff_pix += (pred * mask).sum().item() / mask.sum().item()
        pur_pix += (pred * mask).sum().item() / pred.sum().item()
        eff_roi += (pred.sum() > 0) and (mask.sum() > 0)
        pur_roi += (pred.sum() > 0) / (mask.sum() > 0)
